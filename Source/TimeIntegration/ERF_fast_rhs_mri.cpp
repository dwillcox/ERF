...
void erf_mri_fast_rhs (int level,
                   ...
                   const amrex::Real dtau, const amrex::Real facinv)
{
    ...
    // Initialize New_rho_u/v/w to Current rho_u/v/w so we initialize the ghost cells
    MultiFab::Copy(New_rho_u, S_data[IntVar::xmom], 0, 0, 1, 1);
    MultiFab::Copy(New_rho_v, S_data[IntVar::ymom], 0, 0, 1, 1);
    MultiFab::Copy(New_rho_w, S_data[IntVar::zmom], 0, 0, 1, 1);
